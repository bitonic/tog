  ...
  where
    ...
    go fields fieldTypes = case (fields, fieldTypes) of
      ...
      (proj : fields', (_, fieldType) :> fieldTypes') -> do
        endType <- (`pi` fieldType) =<< ctxApp (def (Opened tyCon []) []) tyConPars
        addProjection proj tyCon $ Contextual (ctxToTel tyConPars) endType
