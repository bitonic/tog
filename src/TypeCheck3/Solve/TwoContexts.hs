...
solve' (Check ctx type_ term) = do
  coreCheckOrPostpone ctx type_ term
...
coreCheckOrPostpone
  :: (IsTerm t)
  => Ctx t -> Type t -> Term t -> TC t s (Constraints t)
coreCheckOrPostpone ctx type_ term = do
  mbErr <- catchTC $ Core.check ctx term type_
  case mbErr of
    Left _err -> do
      error "TODO blah blah"
      -- mvs1 <- metaVarsTC type_
      -- mvs2 <- metaVarsTC term
      -- return [(mvs1 <> mvs2, Check ctx type_ term)]
    Right () -> do
      return []
...
checkedInstantiateMetaVar mv mvT = do
  ...
  coreCheckOrPostpone Ctx.Empty mvType mvT
